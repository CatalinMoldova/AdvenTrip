...
import { FeedSelectionModal } from './components/FeedSelectionModal';
...
function FeedPage() {
  const { user, savedTrips, setSavedTrips, discardedTrips, setDiscardedTrips, adventureRequests, setAdventureRequests } = useUser();
  const navigate = useNavigate();
  const [currentFeed, setCurrentFeed] = useState('for-you');
  const [currentAdventureRequest, setCurrentAdventureRequest] = useState<AdventureRequest | undefined>(undefined);
  const [showFeedSelection, setShowFeedSelection] = useState(false);

  const handleSaveTrip = (adventure: Adventure, rating: number, feedType?: string, adventureRequest?: AdventureRequest) => {
    console.log('handleSaveTrip called:', { adventure: adventure.title, feedType, adventureRequest: adventureRequest?.name });
    
    if (feedType === 'for-you') {
      // Save to profile
      setSavedTrips(prev => {
        if (prev.find(item => item.adventure.id === adventure.id)) {
          console.log('Trip already exists in profile, skipping');
          return prev;
        }
        console.log('Adding trip to profile');
        return [...prev, { adventure, rating }];
      });
    } else if (adventureRequest) {
      // Save to adventure folder
      console.log('Adding trip to adventure folder:', adventureRequest.name);
      setAdventureRequests(prev => 
        prev.map(req => 
          req.id === adventureRequest.id 
            ? {
                ...req,
                savedAdventures: [
                  ...(req.savedAdventures || []).filter(item => item.adventure.id !== adventure.id),
                  { adventure, rating }
                ]
              }
            : req
  ...
  const handleFeedSelection = (feedType: string, adventureRequest?: AdventureRequest) => {
    setCurrentFeed(feedType);
    setCurrentAdventureRequest(adventureRequest);
  };

  const handleFeedLongPress = () => {
    ...
    <>
      <div className="relative pb-20">
        <FeedTab 
          ...
          currentAdventureRequest={currentAdventureRequest}
        />
      </div>
      <BottomTabNavigation
        activeTab="feed"
        ...
        onFeedLongPress={handleFeedLongPress}
      />

      {/* Feed Selection Modal */}
      <FeedSelectionModal
        isOpen={showFeedSelection}
        onClose={() => setShowFeedSelection(false)}
