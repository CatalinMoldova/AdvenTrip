...
export function CreateAdventureWizard({ isOpen, onClose, onCreateAdventure, user }: CreateAdventureWizardProps) {
  ...
  const totalSteps = mode === 'individual' ? 7 : 8;
  ...
  const handleNext = () => {
    // Validation based on new flow
    if (step === 0 && !mode) {
      toast.error('Please select an adventure type');
      return;
    }
    if (step === 1 && mode === 'group' && !adventureName.trim()) {
      toast.error('Please enter an adventure name');
      return;
    }
    if (step === 1 && mode === 'individual') {
      // Skip to location step for solo adventures
      setStep(2);
      return;
    }
    if (step === 2 && mode === 'group' && !name.trim()) {
      toast.error('Please enter your name');
      return;
    }
    if (step === 2 && mode === 'individual' && (!location.trim() || !isLocationValid)) {
      toast.error('Please select a valid location from the suggestions');
      return;
    }
    if (step === 3 && mode === 'group' && (!location.trim() || !isLocationValid)) {
      toast.error('Please select a valid location from the suggestions');
      return;
    }
    if (step === 3 && mode === 'individual' && activities.length === 0) {
      toast.error('Please select at least one activity');
      return;
    }
    if (step === 4 && mode === 'group' && activities.length === 0) {
      toast.error('Please select at least one activity');
      return;
    }
    if (step === 4 && mode === 'individual' && !season) {
      toast.error('Please select a season');
      return;
    }
    if (step === 5 && mode === 'group' && !season) {
      toast.error('Please select a season');
      return;
    }
    if (step === 5 && mode === 'individual' && (!duration || parseInt(duration) < 1)) {
      toast.error('Please enter a valid duration');
      return;
    }
    if (step === 6 && mode === 'group' && (!duration || parseInt(duration) < 1)) {
      toast.error('Please enter a valid duration');
      return;
    }
    if (step === 6 && mode === 'individual' && !transport) {
  ...
  const renderStep = () => {
    switch (step) {
            ...
            {!mode ? (
              ...
              <div className="text-center">
                <div className="text-lg text-black mb-2">
                  {mode === 'individual' ? 'Solo Adventure' : 'Group Adventure'}
                </div>
                <button
      ...
      case 1:
        ...
        // For individual adventures, skip to location step
        return null;
