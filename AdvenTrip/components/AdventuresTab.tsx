...
export function AdventuresTab({ 
  ...
  // Update folders whenever adventureRequests changes
  useEffect(() => {
    setFolders(prevFolders => {
      // Create a map of existing folders to preserve savedAdventures and discardedAdventures
      const existingFoldersMap = new Map(prevFolders.map(f => [f.id, f]));
      
      return adventureRequests.map(req => {
        const existingFolder = existingFoldersMap.get(req.id);
        return {
          id: req.id,
          ...
          savedAdventures: existingFolder?.savedAdventures || [],
          discardedAdventures: existingFolder?.discardedAdventures || [],
              ...
              {folders.map((folder) => {
                ...
                return (
                  <motion.button
                    key={folder.id}
                    onClick={() => {
                      // Navigate to settings page instead of showing feed
                      navigate(`/adventure/${folder.id}/settings`);
                    }}
